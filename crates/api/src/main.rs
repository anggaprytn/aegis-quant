...
#[derive(Deserialize)]
struct ResearchCandidateShadowRunsQueryParams {
    start_time: Option<DateTime<Utc>>,
    end_time: Option<DateTime<Utc>>,
    limit: Option<i64>,
}
...
struct StrategyCandidateObservationEvaluateRequest {
    ...
    min_shadow_runs: Option<i64>,
    max_risk_rejection_rate: Option<Decimal>,
    min_would_submit_count: Option<i64>,
...
#[derive(Serialize)]
struct ResearchCandidateShadowPromotionPreviewResponse {
...
#[derive(Serialize)]
struct ResearchCandidateShadowPromotionResultResponse {
...
#[derive(Serialize)]
struct ResearchCandidateShadowRunsResponse {
    runs: Vec<ResearchCandidateShadowRunLink>,
    request_id: String,
    correlation_id: String,
    timestamp: chrono::DateTime<Utc>,
}
...
async fn main() {
    ...
    let app = Router::new()
        ...
        .route(
            ...
            post(evaluate_strategy_candidate_observation_handler),
        )
        .route(
        ...
        .route(
            ...
            post(preview_research_candidate_shadow_promotion_handler),
        )
        .route(
            "/research/candidates/:id/promote-shadow/apply",
            post(apply_research_candidate_shadow_promotion_handler),
        )
        .route(
            "/research/candidate-observations/:id",
            get(get_strategy_candidate_observation_handler),
        )
        .route(
...
fn route_access(method: &axum::http::Method, path: &str, protect_metrics: bool) -> RouteAccess {
    ...
    if method == axum::http::Method::POST
        ...
        && path.ends_with("/promote-shadow/preview")
    {
        return RouteAccess::Operator;
    }
    if method == axum::http::Method::POST
        && path.starts_with("/research/candidates/")
        && path.ends_with("/promote-shadow/apply")
    {
        return RouteAccess::Owner;
    }
    if method == axum::http::Method::POST
...
fn bounded_research_candidate_shadow_run_limit(limit: Option<i64>) -> i64 {
    match limit {
...
fn candidate_shadow_window_start(
    candidate: &ResearchCandidate,
    start_time: Option<DateTime<Utc>>,
) -> DateTime<Utc> {
    start_time.unwrap_or(candidate.updated_at)
}
...
async fn build_research_candidate_qualification(
    ...
    now: DateTime<Utc>,
) -> anyhow::Result<ResearchCandidateQualificationResult> {
    let latest_observation =
        get_latest_strategy_candidate_observation(&state.db_pool, candidate.id)
            .await?
    ...
    let runner_mismatch_count = if runner_alignment_valid { 0 } else { 1 };
...
fn describe_shadow_runner_config_changes(
...
async fn build_research_candidate_shadow_promotion_preview(
    ...
    let current_runner_hash = serde_json::to_value(&snapshot.config)
        .ok()
        .and_then(|value| hash_json_value(&value).ok());
    ...
    match latest_observation.as_ref() {
        Some(observation) => {
            ...
            if observation
                .observation_snapshot_hash
                .as_deref()
                .zip(current_runner_hash.as_deref())
    ...
    Ok(ResearchCandidateShadowPromotionPreview {
        ...
        correlation_id,
        mode: payload.mode,
        allow_missing_runner_alignment: payload.allow_missing_runner_alignment,
    })
}

fn preview_to_shadow_promotion_result(
    preview: ResearchCandidateShadowPromotionPreview,
...
async fn evaluate_strategy_candidate_observation_handler(
    State(state): State<AppState>,
    Path(candidate_id): Path<Uuid>,
    request: Option<Extension<RequestContext>>,
    actor: Option<Extension<AuthenticatedActor>>,
    Json(payload): Json<StrategyCandidateObservationEvaluateRequest>,
) -> impl IntoResponse {
    let request = request_context(request);
    let actor = match current_actor(actor) {
        Some(value) if matches!(value.role, UserRole::Owner | UserRole::Operator) => value,
        _ => {
            return (
                ...
                Json(ErrorResponse {
                    error: "forbidden",
                    message: "Only OPERATOR or OWNER can evaluate candidate observations."
                        .to_string(),
                    request_id: request.request_id,
                    correlation_id: request.correlation_id,
                    timestamp: Utc::now(),
    ...
    let observation_request = StrategyCandidateObservationRequest {
        candidate_id,
        start_time: payload.start_time,
        min_observation_hours: payload.min_observation_hours.unwrap_or(24),
        min_shadow_runs: payload.min_shadow_runs.unwrap_or(30),
        max_risk_rejection_rate: payload.max_risk_rejection_rate,
        min_would_submit_count: payload.min_would_submit_count.unwrap_or(1),
        max_no_signal_rate: payload.max_no_signal_rate,
        require_readiness_ready: payload.require_readiness_ready.unwrap_or(true),
        correlation_id: payload
            .correlation_id
            .or_else(|| Some(parse_correlation_id(&request.correlation_id))),
    };
...
async fn list_strategy_candidate_observations_handler(
    State(state): State<AppState>,
    Path(candidate_id): Path<Uuid>,
    request: Option<Extension<RequestContext>>,
) -> impl IntoResponse {
    ...
    match list_strategy_candidate_observations(&state.db_pool, candidate_id).await {
        ...
        Err(err) => (
            StatusCode::INTERNAL_SERVER_ERROR,
            Json(ErrorResponse {
                error: "failed_to_list_candidate_observations",
                message: err.to_string(),
                request_id: request.request_id,
                correlation_id: request.correlation_id,
                timestamp: Utc::now(),
...
async fn get_research_candidate_shadow_performance_handler(
    State(state): State<AppState>,
    Path(candidate_id): Path<Uuid>,
    Query(query): Query<ResearchCandidateShadowRunsQueryParams>,
    request: Option<Extension<RequestContext>>,
) -> impl IntoResponse {
    let request = request_context(request);
    let now = Utc::now();
    ...
    let runner_alignment_current =
        load_testnet_shadow_runner_snapshot(&shadow_runtime_state(&state))
            .await
            .map(|snapshot| candidate_runner_alignment_current(&candidate, &snapshot.config))
            .unwrap_or(false);
    let window = ResearchCandidateShadowPerformanceWindow {
        start_time: candidate_shadow_window_start(&candidate, query.start_time),
        end_time: query.end_time.unwrap_or(now),
    };

    match get_research_candidate_shadow_performance(
        &state.db_pool,
        &candidate,
        &window,
    ...
    .await
    {
        Ok(performance) => (
            StatusCode::OK,
            Json(ResearchCandidateShadowPerformanceResponse {
                performance,
                request_id: request.request_id,
                correlation_id: request.correlation_id,
                timestamp: now,
...
async fn get_research_candidate_qualification_handler(
    State(state): State<AppState>,
    Path(candidate_id): Path<Uuid>,
    Query(query): Query<ResearchCandidateQualificationQueryParams>,
    request: Option<Extension<RequestContext>>,
) -> impl IntoResponse {
    let request = request_context(request);
    let now = Utc::now();
    ...
    match build_research_candidate_qualification(
        &state,
        &candidate,
        qualification_thresholds_from_query(&query),
        now,
    )
    .await
    {
        Ok(qualification) => (
            StatusCode::OK,
            Json(ResearchCandidateQualificationResponse {
                qualification,
                request_id: request.request_id,
                correlation_id: request.correlation_id,
                timestamp: now,
...
async fn list_research_candidate_shadow_runs_handler(
    State(state): State<AppState>,
    Path(candidate_id): Path<Uuid>,
    Query(query): Query<ResearchCandidateShadowRunsQueryParams>,
    request: Option<Extension<RequestContext>>,
    ...
    let query = ResearchCandidateShadowRunsQuery {
        start_time: query.start_time.unwrap_or_else(|| {
            Utc.timestamp_opt(0, 0)
                .single()
                .expect("unix epoch is valid")
        }),
        end_time: query.end_time.unwrap_or(now),
        limit: bounded_research_candidate_shadow_run_limit(query.limit),
    };

    match list_research_candidate_shadow_runs(&state.db_pool, candidate_id, &query).await {
        Ok(runs) => (
            StatusCode::OK,
            Json(ResearchCandidateShadowRunsResponse {
                runs,
                request_id: request.request_id,
                correlation_id: request.correlation_id,
                timestamp: now,
        ...
        Err(err) => (
            StatusCode::INTERNAL_SERVER_ERROR,
            Json(ErrorResponse {
                error: "failed_to_list_research_candidate_shadow_runs",
                message: err.to_string(),
                request_id: request.request_id,
                correlation_id: request.correlation_id,
                timestamp: now,
...
async fn get_strategy_candidate_observation_handler(
    State(state): State<AppState>,
    Path(observation_id): Path<Uuid>,
    request: Option<Extension<RequestContext>>,
) -> impl IntoResponse {
    let request = request_context(request);
    match db::get_strategy_candidate_observation(&state.db_pool, observation_id).await {
        Ok(Some(record)) => match strategy_candidate_observation_result_from_record(&record) {
            Ok(observation) => (
                StatusCode::OK,
                Json(StrategyCandidateObservationResponse {
                    observation,
            ...
            Err(err) => (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(ErrorResponse {
                    error: "failed_to_map_candidate_observation",
                    message: err.to_string(),
                    request_id: request.request_id,
                    correlation_id: request.correlation_id,
                    timestamp: Utc::now(),
        ...
        Ok(None) => (
            StatusCode::NOT_FOUND,
            Json(ErrorResponse {
                error: "candidate_observation_not_found",
                message: "Candidate observation was not found.".to_string(),
                request_id: request.request_id,
                correlation_id: request.correlation_id,
                timestamp: Utc::now(),
            }),
        ...
        Err(err) => (
            StatusCode::INTERNAL_SERVER_ERROR,
            Json(ErrorResponse {
                error: "failed_to_query_candidate_observation",
                message: err.to_string(),
                request_id: request.request_id,
                correlation_id: request.correlation_id,
                timestamp: Utc::now(),
...
async fn preview_research_candidate_shadow_promotion_handler(
    State(state): State<AppState>,
    Path(candidate_id): Path<Uuid>,
    request: Option<Extension<RequestContext>>,
    actor: Option<Extension<AuthenticatedActor>>,
    Json(payload): Json<ResearchCandidateShadowPromotionRequest>,
) -> impl IntoResponse {
    let request = request_context(request);
    let _actor = match current_actor(actor) {
        Some(value) if matches!(value.role, UserRole::Owner | UserRole::Operator) => value,
        _ => {
            return (
                StatusCode::FORBIDDEN,
                Json(ErrorResponse {
                    error: "forbidden",
                    message: "Only OPERATOR or OWNER can preview shadow promotion.".to_string(),
                    request_id: request.request_id,
                    correlation_id: request.correlation_id,
                    timestamp: Utc::now(),
                }),
    ...
    if payload.mode != ResearchCandidateShadowPromotionMode::PreviewOnly {
        return (
            StatusCode::BAD_REQUEST,
            Json(ErrorResponse {
                error: "invalid_promotion_mode",
                message: "Preview endpoint requires mode PREVIEW_ONLY.".to_string(),
                request_id: request.request_id,
                correlation_id: request.correlation_id,
                timestamp: Utc::now(),
            }),
    ...
    match build_research_candidate_shadow_promotion_preview(
        &state,
        &candidate,
        &payload,
        correlation_id,
    ...
    .await
    {
        Ok(preview) => (
            StatusCode::OK,
            Json(ResearchCandidateShadowPromotionPreviewResponse {
                preview,
                request_id: request.request_id,
                correlation_id: request.correlation_id,
                timestamp: Utc::now(),
        ...
        Err(err) => (
            StatusCode::INTERNAL_SERVER_ERROR,
            Json(ErrorResponse {
                error: "failed_to_preview_shadow_promotion",
                message: err.to_string(),
                request_id: request.request_id,
                correlation_id: request.correlation_id,
                timestamp: Utc::now(),
...
async fn apply_research_candidate_shadow_promotion_handler(
    State(state): State<AppState>,
    Path(candidate_id): Path<Uuid>,
    request: Option<Extension<RequestContext>>,
    actor: Option<Extension<AuthenticatedActor>>,
    ...
    let actor = match current_actor(actor) {
        ...
        _ => {
            return (
                ...
                Json(ErrorResponse {
                    error: "forbidden",
                    message: "Only OWNER can apply shadow promotion while runner config updates remain owner-gated."
                        .to_string(),
                    request_id: request.request_id,
                    correlation_id: request.correlation_id,
                    timestamp: Utc::now(),
    ...
    if payload.mode != ResearchCandidateShadowPromotionMode::Apply {
        return (
            StatusCode::BAD_REQUEST,
            Json(ErrorResponse {
                error: "invalid_promotion_mode",
                message: "Apply endpoint requires mode APPLY.".to_string(),
                request_id: request.request_id,
                correlation_id: request.correlation_id,
                timestamp: Utc::now(),
            }),
    ...
    let correlation_id = payload
        .correlation_id
        .unwrap_or_else(|| parse_correlation_id(&request.correlation_id));
    if !is_valid_research_candidate_shadow_promotion_confirmation(
        candidate_id,
        payload.confirmation_text.as_deref().unwrap_or(""),
    ) {
        return (
            StatusCode::BAD_REQUEST,
            Json(ErrorResponse {
                error: "invalid_confirmation_text",
                message: format!(
                    "Shadow promotion requires confirmation_text exactly equal to {:?}.",
                    expected_research_candidate_shadow_promotion_confirmation(candidate_id)
                ),
                request_id: request.request_id,
                correlation_id: request.correlation_id,
                timestamp: Utc::now(),
    ...
    let preview = match build_research_candidate_shadow_promotion_preview(
        &state,
        &candidate,
        &payload,
        correlation_id,
    ...
    .await
    {
        ...
        Err(err) => {
            return (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(ErrorResponse {
                    error: "failed_to_prepare_shadow_promotion",
                    message: err.to_string(),
                    request_id: request.request_id,
                    correlation_id: request.correlation_id,
                    timestamp: Utc::now(),
    ...
    if preview.status == ResearchCandidateShadowPromotionStatus::Blocked {
        return (
            StatusCode::CONFLICT,
            Json(ResearchCandidateShadowPromotionResultResponse {
                result: preview_to_shadow_promotion_result(
                    preview,
                    ResearchCandidateShadowPromotionStatus::Blocked,
                    false,
                ),
    ...
    let state_actor = state_actor_from_authenticated(&actor);
    let applied = if preview.changes.is_empty() {
        false
    } else {
        match persist_testnet_shadow_runner_config(
            &shadow_runtime_state(&state),
            &TestnetShadowRunnerConfigInput {
                enabled: preview.proposed_runner_config.enabled,
                interval_seconds: preview.proposed_runner_config.interval_seconds,
                strategies: preview.proposed_runner_config.strategies.clone(),
                symbols: preview.proposed_runner_config.symbols.clone(),
                timeframe: preview.proposed_runner_config.timeframe.clone(),
                max_runs_per_tick: preview.proposed_runner_config.max_runs_per_tick,
                stale_feed_policy: preview.proposed_runner_config.stale_feed_policy,
                notes: preview.proposed_runner_config.notes.clone(),
            },
            Some(actor.user_id),
        )
        .await
        {
            ...
            Err(err) => {
                return (
                    StatusCode::INTERNAL_SERVER_ERROR,
                    Json(ErrorResponse {
                        error: "failed_to_update_shadow_runner_config",
                        message: err.to_string(),
                        request_id: request.request_id,
                        correlation_id: request.correlation_id,
                        timestamp: Utc::now(),
...
mod tests {
    ...
    use aegis_core::{
        ...
        ResearchCandidateQualificationStatus, ResearchCandidateStatus, RiskConfig, Side,
    ...
    use db::{
        ...
        mark_strategy_research_candidate_promoted, set_kill_switch_state,
        ...
        upsert_testnet_shadow_runner_config, upsert_testnet_shadow_runner_state,
    ...
    fn research_shadow_promotion_route_access_matches_role_expectations() {
        assert!(matches!(
            route_access(
                &axum::http::Method::POST,
                "/research/candidates/123e4567-e89b-12d3-a456-426614174000/promote-shadow/preview",
                false
            ),
            super::RouteAccess::Operator
        ));
    ...
    fn shadow_promotion_proposed_runner_config_preserves_existing_entries_and_appends_missing() {
        ...
        let candidate = ResearchCandidate {
            ...
            win_rate: None,
            fee_drag: None,
            status: ResearchCandidateStatus::AcceptedForShadow,
            rejection_reason: None,
            notes: None,
            created_at: Utc::now(),
            updated_at: Utc::now(),
    ...
    fn sample_lifecycle_research_candidate(
        strategy: &StrategyConfig,
        candidate_id: Uuid,
    ...
    async fn count_research_candidate_observations(pool: &PgPool) -> i64 {
        sqlx::query("SELECT COUNT(*) AS count FROM strategy_candidate_observations")
            .fetch_one(pool)
            .await
            .expect("research candidate observation count")
            .get::<i64, _>("count")
    }
    ...
    async fn assert_observation_execution_unchanged(
        pool: &PgPool,
        before: (i64, i64, i64, i64, i64),
    ) {
        assert_eq!(count_paper_orders(pool).await, before.0);
    ...
    async fn observation_mutation_counts(pool: &PgPool) -> (i64, i64, i64, i64, i64) {
        readiness_mutation_counts(pool).await
    }

    async fn research_shadow_promotion_execution_counts(
        pool: &PgPool,
    ) -> (i64, i64, i64, i64, i64, i64) {
        (
            count_paper_orders(pool).await,
            count_paper_positions(pool).await,
            count_paper_fills(pool).await,
            count_exchange_testnet_orders(pool).await,
            count_exchange_testnet_lifecycle_events(pool).await,
            count_testnet_shadow_promotions(pool).await,
        )
    }

    async fn assert_research_shadow_promotion_execution_unchanged(
        pool: &PgPool,
        before: (i64, i64, i64, i64, i64, i64),
    ) {
        assert_eq!(count_paper_orders(pool).await, before.0);
        assert_eq!(count_paper_positions(pool).await, before.1);
        assert_eq!(count_paper_fills(pool).await, before.2);
        assert_eq!(count_exchange_testnet_orders(pool).await, before.3);
        assert_eq!(
            count_exchange_testnet_lifecycle_events(pool).await,
            before.4
        );
    ...
    #[tokio::test]
    #[ignore = "requires TEST_DATABASE_URL or DATABASE_URL pointing to a test database"]
    async fn research_candidate_observation_persists_lists_gets_and_avoids_execution_mutation() {
        let test_db = TestDatabase::setup().await.expect("test db");
        let state = auth_test_state(test_db.pool.clone(), None, None);
        let app = research_test_router(state);
        let operator_email = "research-operator-observe@example.com";
        let viewer_email = "research-viewer-observe@example.com";
        insert_test_user(
            &test_db.pool,
            operator_email,
            "replace-with-a-12-char-min-password",
        ...
        let payload = response_json::<Value>(evaluate).await;
        let observation_id = payload["observation"]["observation_id"]
            .as_str()
            .expect("observation id");
        assert_eq!(
            count_research_candidate_observations(&test_db.pool).await,
            1
        );
        ...
        let list_payload = response_json::<Value>(listed).await;
        assert_eq!(
            list_payload["observations"]
                .as_array()
        ...
        let fetched = app
            .oneshot(
                Request::builder()
                    .method("GET")
                    .uri(format!("/research/candidate-observations/{observation_id}"))
                    .header(
                        AUTHORIZATION,
                        format!("Bearer {}", viewer_login.access_token),
                    )
        ...
        let fetched_payload = response_json::<Value>(fetched).await;
        assert_eq!(
    ...
    #[tokio::test]
    #[ignore = "requires TEST_DATABASE_URL or DATABASE_URL pointing to a test database"]
    async fn research_candidate_shadow_endpoints_return_only_linked_runs_without_execution_mutation(
    ) {
        let test_db = TestDatabase::setup().await.expect("test db");
        let state = auth_test_state(test_db.pool.clone(), None, None);
        let app = research_test_router(state);
        let viewer_email = "research-viewer-shadow-performance@example.com";
        insert_test_user(
            &test_db.pool,
            viewer_email,
            "replace-with-a-12-char-min-password",
        ...
        let linked_run = TestnetShadowRunRecord {
            id: Uuid::new_v4(),
            strategy_id: "momentum_v1".to_string(),
            symbol: "BTCUSDT".to_string(),
            timeframe: "15m".to_string(),
        ...
        let unlinked_run = TestnetShadowRunRecord {
            id: Uuid::new_v4(),
            strategy_id: "momentum_v1".to_string(),
            symbol: "BTCUSDT".to_string(),
            timeframe: "15m".to_string(),
            decision: "RISK_REJECTED".to_string(),
            signal_id: None,
            risk_decision_id: None,
            would_submit_payload: None,
            price_source: Some("local".to_string()),
        ...
        insert_testnet_shadow_run(&test_db.pool, &linked_run)
            .await
            .expect("linked run should persist");
        insert_testnet_shadow_run(&test_db.pool, &unlinked_run)
            .await
            .expect("unlinked run should persist");
        ...
        let performance = app
            ...
            .oneshot(
                Request::builder()
                    .method("GET")
                    .uri(format!(
                        "/research/candidates/{}/shadow-performance",
                        candidate.id
                    ))
                    .header(
                        AUTHORIZATION,
        ...
        let performance_payload = response_json::<Value>(performance).await;
        assert_eq!(performance_payload["performance"]["total_shadow_runs"], 1);
        assert_eq!(performance_payload["performance"]["would_submit_count"], 1);
        ...
        let runs = app
            .oneshot(
                Request::builder()
                    .method("GET")
                    .uri(format!(
                        "/research/candidates/{}/shadow-runs?limit=50",
                        candidate.id
                    ))
                    .header(
                        AUTHORIZATION,
        ...
        let runs_payload = response_json::<Value>(runs).await;
    ...
    #[tokio::test]
    #[ignore = "requires TEST_DATABASE_URL or DATABASE_URL pointing to a test database"]
    async fn research_candidate_qualification_returns_needs_more_data_with_zero_linked_runs_and_no_execution_mutation(
    ) {
        let test_db = TestDatabase::setup().await.expect("test db");
        let state = auth_test_state(test_db.pool.clone(), None, None);
        let app = research_test_router(state);
        let viewer_email = "research-viewer-qualification-zero@example.com";
        insert_test_user(
            &test_db.pool,
            viewer_email,
            "replace-with-a-12-char-min-password",
        ...
        assert_eq!(response.status(), StatusCode::OK);
        let payload = response_json::<Value>(response).await;
        assert_eq!(payload["qualification"]["status"], "NEEDS_MORE_DATA");
        assert_eq!(
            payload["qualification"]["shadow_performance"]["total_shadow_runs"],
            0
        );
    ...
    #[tokio::test]
    #[ignore = "requires TEST_DATABASE_URL or DATABASE_URL pointing to a test database"]
    async fn research_candidate_qualification_supports_qualified_degraded_and_threshold_override_paths(
    ) {
        let test_db = TestDatabase::setup().await.expect("test db");
        let state = auth_test_state(test_db.pool.clone(), None, None);
        let app = research_test_router(state);
        let viewer_email = "research-viewer-qualification-paths@example.com";
        insert_test_user(
            &test_db.pool,
            viewer_email,
            "replace-with-a-12-char-min-password",
        ...
        let qualified_candidate = sample_lifecycle_research_candidate(
            &research_strategy_config(CandleInterval::FifteenMinutes, "BTCUSDT"),
            Uuid::new_v4(),
            ResearchCandidateStatus::AcceptedForShadow,
        );
        create_research_candidate(
            ...
            &json!({"fixture": "qualified"}),
        )
        .await
        .expect("qualified candidate should persist");

        let mut qualified_observation = sample_observation_for_tests(
            Utc::now(),
            900,
            true,
            Some(ExecutionReadinessStatus::Ready),
        );
        qualified_observation.candidate_id = qualified_candidate.id;
        qualified_observation.strategy_id = qualified_candidate.strategy_id.clone();
        qualified_observation.symbol = qualified_candidate.symbol.clone();
        qualified_observation.timeframe = qualified_candidate.timeframe.clone();
        qualified_observation.requirements.candidate_id = qualified_candidate.id;
        qualified_observation.summary.candidate_id = qualified_candidate.id;
        insert_strategy_candidate_observation(&test_db.pool, &qualified_observation)
            .await
            .expect("qualified observation should persist");

        for index in 0..6 {
            let run = TestnetShadowRunRecord {
                id: Uuid::new_v4(),
                strategy_id: qualified_candidate.strategy_id.clone(),
                symbol: qualified_candidate.symbol.clone(),
                timeframe: qualified_candidate.timeframe.clone(),
                decision: "WOULD_SUBMIT".to_string(),
                signal_id: None,
                risk_decision_id: None,
                would_submit_payload: None,
            ...
            insert_testnet_shadow_run(&test_db.pool, &run)
                .await
                .expect("qualified run should persist");
            insert_research_candidate_shadow_run_link(
                &test_db.pool,
                qualified_candidate.id,
                run.id,
                run.created_at,
            )
            .await
            .expect("qualified link should persist");
        }

        let degraded_candidate = sample_lifecycle_research_candidate(
            &research_strategy_config(CandleInterval::FifteenMinutes, "BTCUSDT"),
            Uuid::new_v4(),
            ResearchCandidateStatus::AcceptedForShadow,
        );
        create_research_candidate(
            ...
            ResearchCandidateDecision::AcceptForShadow,
            Some("fixture"),
            Some("degraded fixture"),
            &json!({"fixture": "degraded"}),
        )
        .await
        .expect("degraded candidate should persist");

        let mut degraded_observation = sample_observation_for_tests(
            Utc::now(),
            900,
            true,
            Some(ExecutionReadinessStatus::Ready),
        );
        degraded_observation.candidate_id = degraded_candidate.id;
        degraded_observation.strategy_id = degraded_candidate.strategy_id.clone();
        degraded_observation.symbol = degraded_candidate.symbol.clone();
        degraded_observation.timeframe = degraded_candidate.timeframe.clone();
        degraded_observation.requirements.candidate_id = degraded_candidate.id;
        degraded_observation.summary.candidate_id = degraded_candidate.id;
        insert_strategy_candidate_observation(&test_db.pool, &degraded_observation)
            .await
            .expect("degraded observation should persist");
        ...
        for index in 0..5 {
            ...
            let status = if decision == "RISK_REJECTED" {
                "REJECTED"
            } else {
                "COMPLETED"
            };
            let run = TestnetShadowRunRecord {
                id: Uuid::new_v4(),
                strategy_id: degraded_candidate.strategy_id.clone(),
                symbol: degraded_candidate.symbol.clone(),
                timeframe: degraded_candidate.timeframe.clone(),
            ...
            insert_testnet_shadow_run(&test_db.pool, &run)
                .await
                .expect("degraded run should persist");
            insert_research_candidate_shadow_run_link(
                &test_db.pool,
                degraded_candidate.id,
                run.id,
                run.created_at,
            )
            .await
            .expect("degraded link should persist");
        }
        ...
        let qualified = app
            ...
            .oneshot(
                Request::builder()
                    .method("GET")
                    .uri(format!(
                        "/research/candidates/{}/qualification?min_shadow_runs=5&min_would_submit_count=3",
                        qualified_candidate.id
                    ))
                    .header(
                        AUTHORIZATION,
        ...
        let qualified_payload = response_json::<Value>(qualified).await;
        assert_eq!(qualified_payload["qualification"]["status"], "QUALIFIED");
        assert_eq!(
            qualified_payload["qualification"]["thresholds"]["min_shadow_runs"],
            5
        );
        assert_eq!(qualified_payload["qualification"]["score"], 95);
        assert_eq!(
            qualified_payload["qualification"]["latest_readiness_status"],
            "READY"
        );
        assert_eq!(
            qualified_payload["qualification"]["threshold_override_below_default"],
            true
        );
        assert!(qualified_payload["qualification"]["warnings"]
            .as_array()
            .expect("warnings array")
            .iter()
            .any(|item| item.as_str().unwrap_or_default().contains("below default")));
        ...
        let degraded = app
            .oneshot(
                Request::builder()
                    .method("GET")
                    .uri(format!(
                        "/research/candidates/{}/qualification?min_shadow_runs=5&min_would_submit_count=2",
                        degraded_candidate.id
                    ))
                    .header(
                        AUTHORIZATION,
        ...
        let degraded_payload = response_json::<Value>(degraded).await;
        assert_eq!(degraded_payload["qualification"]["status"], "DEGRADED");
        assert!(degraded_payload["qualification"]["warnings"]
            .as_array()
            .expect("warnings array")
            .iter()
            .any(|item| item
    ...
    #[tokio::test]
    #[ignore = "requires TEST_DATABASE_URL or DATABASE_URL pointing to a test database"]
    async fn operator_report_includes_research_candidate_qualification_section_without_execution_mutation(
    ) {
        let test_db = TestDatabase::setup().await.expect("test db");
        let state = auth_test_state(test_db.pool.clone(), None, None);
        let app = Router::new()
            ...
            .with_state(state);
        let viewer_email = "research-viewer-operator-report@example.com";
        insert_test_user(
            &test_db.pool,
            viewer_email,
            "replace-with-a-12-char-min-password",
        ...
        create_research_candidate(
            ...
            ResearchCandidateDecision::AcceptForShadow,
            Some("fixture"),
            Some("accepted candidate needs more data"),
            &json!({"fixture": "report-degraded"}),
        )
        .await
        .expect("degraded candidate should persist");
        ...
        let not_ready_candidate = sample_lifecycle_research_candidate(
            &research_strategy_config(CandleInterval::FifteenMinutes, "BTCUSDT"),
            Uuid::new_v4(),
            ResearchCandidateStatus::AcceptedForShadow,
        );
        create_research_candidate(
            ...
            ResearchCandidateDecision::AcceptForShadow,
            Some("fixture"),
            Some("accepted candidate blocked by readiness"),
            &json!({"fixture": "report-not-ready"}),
        )
        .await
        .expect("not-ready candidate should persist");

        let mut not_ready_observation = sample_observation_for_tests(
            Utc::now(),
            900,
            true,
            Some(ExecutionReadinessStatus::NotReady),
        );
        not_ready_observation.candidate_id = not_ready_candidate.id;
        not_ready_observation.strategy_id = not_ready_candidate.strategy_id.clone();
        not_ready_observation.symbol = not_ready_candidate.symbol.clone();
        not_ready_observation.timeframe = not_ready_candidate.timeframe.clone();
        not_ready_observation.requirements.candidate_id = not_ready_candidate.id;
        not_ready_observation.summary.candidate_id = not_ready_candidate.id;
        insert_strategy_candidate_observation(&test_db.pool, &not_ready_observation)
            .await
            .expect("not-ready observation should persist");
        ...
        let response = app
            .oneshot(
                Request::builder()
                    ...
                    .body(Body::from(
                        serde_json::to_vec(&OperatorReportRequest::default())
        ...
        assert!(payload["report"]["sections"]
            ...
            .iter()
            .any(|section| section["title"] == "Research Candidate Qualification"));
        assert!(payload["report"]["findings"]
            .as_array()
            .expect("findings array")
            .iter()
            .any(|item| item["title"] == "No qualified candidates yet"));
        assert!(payload["report"]["findings"]
            .as_array()
            .expect("findings array")
            .iter()
            .any(|item| item["title"] == "Accepted candidates still need more shadow data"));
    ...
    #[tokio::test]
    #[ignore = "requires TEST_DATABASE_URL or DATABASE_URL pointing to a test database"]
    async fn research_candidate_observation_not_promoted_returns_insufficient_data() {
        let test_db = TestDatabase::setup().await.expect("test db");
        let state = auth_test_state(test_db.pool.clone(), None, None);
        let app = research_test_router(state);
        let operator_email = "research-operator-observe-unpromoted@example.com";
        insert_test_user(
            &test_db.pool,
            operator_email,
            "replace-with-a-12-char-min-password",
    ...
    async fn accept_for_shadow_requires_fresh_persisted_observation() {
        ...
        let _observation_id = Uuid::parse_str(
    ...
    #[tokio::test]
    #[ignore = "requires TEST_DATABASE_URL or DATABASE_URL pointing to a test database"]
    async fn research_candidate_shadow_promotion_preview_returns_diff_without_mutation() {
        let test_db = TestDatabase::setup().await.expect("test db");
        let state = auth_test_state(test_db.pool.clone(), None, None);
        let app = research_test_router(state);
        let operator_email = "research-shadow-preview@example.com";
        insert_test_user(
            &test_db.pool,
            operator_email,
            "replace-with-a-12-char-min-password",
        ...
        let before_runner_config = sqlx::query(
            "SELECT strategies, symbols, timeframe, updated_at FROM testnet_shadow_runner_config WHERE id = $1",
        )
        .bind(db::TESTNET_SHADOW_RUNNER_CONFIG_ID)
        .fetch_one(&test_db.pool)
        .await
        ...
        let preview = app
            .oneshot(bearer_request(
                "POST",
                &format!(
                    "/research/candidates/{}/promote-shadow/preview",
                    candidate.id
                ),
                &operator_login.access_token,
                json!({
                    "mode": "PREVIEW_ONLY",
                    "allow_missing_runner_alignment": true
                }),
            ))
            .await
            .expect("preview response");
        ...
        let after_runner_config = sqlx::query(
            "SELECT strategies, symbols, timeframe, updated_at FROM testnet_shadow_runner_config WHERE id = $1",
        )
        .bind(db::TESTNET_SHADOW_RUNNER_CONFIG_ID)
        .fetch_one(&test_db.pool)
        ...
        .expect("runner config row");
        assert_eq!(
            after_runner_config.get::<Value, _>("strategies"),
            before_runner_config.get::<Value, _>("strategies")
        );
        assert_eq!(
            after_runner_config.get::<Value, _>("symbols"),
            before_runner_config.get::<Value, _>("symbols")
        );
        assert_eq!(
            after_runner_config.get::<String, _>("timeframe"),
            before_runner_config.get::<String, _>("timeframe")
        );
        assert_eq!(
            after_runner_config.get::<chrono::DateTime<Utc>, _>("updated_at"),
            before_runner_config.get::<chrono::DateTime<Utc>, _>("updated_at")
        );
    }

    #[tokio::test]
    #[ignore = "requires TEST_DATABASE_URL or DATABASE_URL pointing to a test database"]
    async fn research_candidate_shadow_promotion_blocks_not_accepted_and_stale_observations() {
        let test_db = TestDatabase::setup().await.expect("test db");
        let state = auth_test_state(test_db.pool.clone(), None, None);
        let app = research_test_router(state);
        let operator_email = "research-shadow-blocked@example.com";
        insert_test_user(
            &test_db.pool,
            operator_email,
            "replace-with-a-12-char-min-password",
        ...
        .await
        .expect("runner config");
        let not_accepted = sample_lifecycle_research_candidate(
            &research_strategy_config(CandleInterval::FifteenMinutes, "BTCUSDT"),
            Uuid::new_v4(),
        ...
        create_research_candidate(
            ...
            ResearchCandidateDecision::Reopen,
            Some("fixture"),
            Some("observing fixture"),
            &json!({"fixture": true}),
        )
        .await
        .expect("observing candidate should persist");
        let stale_candidate = sample_lifecycle_research_candidate(
            &research_strategy_config(CandleInterval::FifteenMinutes, "BTCUSDT"),
            Uuid::new_v4(),
            ResearchCandidateStatus::AcceptedForShadow,
        );
        ...
        let not_accepted_preview = app
            ...
            .oneshot(bearer_request(
                ...
                json!({
                    "mode": "PREVIEW_ONLY",
                    "allow_missing_runner_alignment": false
                }),
            ))
            .await
            .expect("not accepted preview");
        assert_eq!(not_accepted_preview.status(), StatusCode::OK);
        let not_accepted_payload = response_json::<Value>(not_accepted_preview).await;
        assert_eq!(not_accepted_payload["preview"]["status"], "BLOCKED");
        assert!(not_accepted_payload["preview"]["reasons"]
            .as_array()
            ...
            .any(|value| value
                .as_str()
                .unwrap_or_default()
                .contains("is not ACCEPTED_FOR_SHADOW")));
        ...
        let observe = app
            ...
            .expect("observe response");
        assert_eq!(observe.status(), StatusCode::OK);
        let observed_payload = response_json::<Value>(observe).await;
        ...
        assert_eq!(stale_preview.status(), StatusCode::OK);
        let stale_payload = response_json::<Value>(stale_preview).await;
        assert_eq!(stale_payload["preview"]["status"], "BLOCKED");
        assert!(stale_payload["preview"]["reasons"]
            .as_array()
            .expect("reasons array")
            .iter()
            .any(|value| value.as_str() == Some("observation_stale")));
    }

    #[tokio::test]
    #[ignore = "requires TEST_DATABASE_URL or DATABASE_URL pointing to a test database"]
    async fn research_candidate_shadow_promotion_apply_requires_exact_confirmation() {
        let test_db = TestDatabase::setup().await.expect("test db");
        let state = auth_test_state(test_db.pool.clone(), None, None);
        let app = research_test_router(state);
        let owner_email = "research-shadow-owner-confirm@example.com";
        insert_test_user(
            &test_db.pool,
            owner_email,
            "replace-with-a-12-char-min-password",
        ...
        let before_runner_config = sqlx::query(
            "SELECT strategies, symbols FROM testnet_shadow_runner_config WHERE id = $1",
        )
        .bind(db::TESTNET_SHADOW_RUNNER_CONFIG_ID)
        .fetch_one(&test_db.pool)
        .await
        ...
        let response = app
            .oneshot(bearer_request(
                "POST",
                &format!("/research/candidates/{}/promote-shadow/apply", candidate.id),
                &owner_login.access_token,
                json!({
                    "mode": "APPLY",
                    "allow_missing_runner_alignment": true,
                    "confirmation_text": "PROMOTE CANDIDATE WRONG TO SHADOW"
                }),
            ))
            .await
    ...
    #[tokio::test]
    #[ignore = "requires TEST_DATABASE_URL or DATABASE_URL pointing to a test database"]
    async fn research_candidate_shadow_promotion_apply_updates_runner_config_and_audits_without_execution_mutation(
    ) {
        let test_db = TestDatabase::setup().await.expect("test db");
        let state = auth_test_state(test_db.pool.clone(), None, None);
        let app = research_test_router(state);
        let owner_email = "research-shadow-owner-apply@example.com";
        insert_test_user(
            &test_db.pool,
            owner_email,
            "replace-with-a-12-char-min-password",
        ...
        let before_execution = research_shadow_promotion_execution_counts(&test_db.pool).await;
        let before_events = sqlx::query(
            "SELECT COUNT(*) AS count FROM research_candidate_events WHERE candidate_id = $1",
        )
        .bind(candidate.id)
        .fetch_one(&test_db.pool)
        .await
        .expect("candidate event count")
        .get::<i64, _>("count");
        let before_audit =
            count_audit_logs(&test_db.pool, "research.candidate.shadow_promotion.applied").await;
        let before_system =
            count_system_events(&test_db.pool, "research.candidate.shadow_promotion.applied").await;
        ...
        let apply = app
            .oneshot(bearer_request(
                ...
                json!({
                    "mode": "APPLY",
                    "allow_missing_runner_alignment": true,
                    "confirmation_text": expected_research_candidate_shadow_promotion_confirmation(candidate.id)
                }),
            ))
            .await
            .expect("apply response");
        assert_eq!(apply.status(), StatusCode::OK);
        let payload = response_json::<Value>(apply).await;
        ...
        assert_eq!(
            count_system_events(&test_db.pool, "research.candidate.shadow_promotion.applied",)
                .await,
            before_system + 1
        );
        assert_eq!(
        ...
        .expect("last event");
        assert_eq!(
            last_event.get::<String, _>("decision"),
